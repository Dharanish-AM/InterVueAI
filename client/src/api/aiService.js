...
export async function generateQuestions(role = "Full-Stack Developer") {
  try {
    const { data } = await axios.post(`${API_BASE_URL}/generate-questions`, { role });
    return data;
  } catch (error) {
...
export async function scoreAnswer(question, answer, difficulty) {
  try {
    const { data } = await axios.post(`${API_BASE_URL}/score-answer`, { question, answer, difficulty });
    return data;
  } catch (error) {
...
export async function generateSummary(candidate, answers) {
  try {
    const { data } = await axios.post(`${API_BASE_URL}/generate-summary`, { candidate, answers });
    return data;
  } catch (error) {
